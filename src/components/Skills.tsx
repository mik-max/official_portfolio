  ...
  return (
    <section id="skills" className="relative py-20 sm:py-28 border-t border-ink/10 bg-paper">
      <div className="section-content max-w-[1400px] mx-auto px-6 lg:px-20">
        ...
        <h2 className="font-display text-4xl sm:text-5xl lg:text-6xl font-normal leading-tight mb-16 perspective-[1000px]">Tools &amp; technologies</h2>
            ...
            <div key={category.label} className="group">
              ...
              <div className="flex flex-wrap gap-2.5 sm:gap-3">
                {category.skills.map((skill) => (
                  <div key={skill} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} className="px-4 py-2 sm:px-5 sm:py-2.5 bg-ink/3 hover:bg-ink/6 border border-ink/10 hover:border-ink/25 rounded-full text-ink/75 text-xs sm:text-sm font-medium transition-all">{skill}</div>
                ))}
              </div>
            </div>
          ))}
